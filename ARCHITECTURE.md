...
| Animation framework | `lib/design/motion.dart`, `test/animation_framework_test.dart` | Implemented (P1.5-10 shared motion tokens: durations, curves, press scale, hover elevation; SectionTransition crossfade+slide for shell navigation; PressableScale button press feedback; InteractiveCard hover+press with elevation and border highlight; StaggeredFadeIn for list items; custom slide+fade PageTransitionsBuilder for Navigator routes; button animationDuration wired through theme) |
...
| Flutter note-highway widget | `lib/features/player/note_highway/`, `test/note_highway_test.dart` | Implemented (P1-09 vertical lane painter, timeline-synced note geometry, grade-colored hit markers; P1.5-11 visual overhaul with top-to-bottom lane gradients, approaching-glow note halos, gradient-filled rounded pills, hit-line glow band, grade-specific burst/ripple feedback markers, miss cross indicator, and past-window note fade-out) |
...
| Flutter visual drum kit widget | `lib/features/player/drum_kit/`, `test/drum_kit_test.dart` | Implemented (P1-11 overhead kit painter, standard 5-piece pad geometry, custom layout adaptation, grade-colored hit flashes; P1.5-12 visual overhaul with distinct cymbal/drum/kick shapes, metallic cymbal fills with bell dots and groove rings, hi-hat pedal indicator, drum rim highlights and tension cross-hairs, kick concentric rings and beater patch, expanding-ring hit flash effect) |
...
```
Native MIDI adapter -> Dart MIDI event -> Rust practice_runtime bridge
       -> RawMidiEvent -> Rust MidiMapper -> MappedHit
       -> InputHit { lane_id, velocity, timestamp_ns, midi_note: Some(raw_note) }

Touch tap pad -> selected lane_id + touch timestamp/velocity
       -> Rust practice_runtime bridge
