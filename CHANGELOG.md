...
### Added
- Global polish pass: 26 tooltips across all screens (practice toolbar, settings buttons, library actions, review actions, play mode), keyboard shortcuts in Practice Mode (Space for play/pause, Escape for stop, M for metronome, L for loop, +/- for tempo), SnackBar feedback for profile switch, profile creation, profile deletion, MIDI scan, and profile name save. Dynamic AppBar title shows current section (e.g., "Taal — Library"). Responsive width verification at 1024px minimum desktop width.
- Tap pad visual and interaction polish: pads now use drum-kit-style shapes (circular for kick, stadium/pill for cymbals, rounded rectangle for drums) matching the overhead kit visualization. Tapping a pad triggers an expanding ring animation with grade-colored feedback when the engine returns a grade. The "Connect your kit for the best experience" guidance banner is now dismissible. All pads retain semantic labels for accessibility.
